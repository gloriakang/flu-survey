...
# 
## svytable(~Q + PPGENDER, des, round = T)
# gen <- p + aes(PPGENDER, fill = Q) + geom_bar(position = "dodge")
# age <- p + aes(ppagect4, fill = Q) + geom_bar(position = "dodge")
# eth <- p + aes(PPETHM, fill = Q) + geom_bar(position = "dodge")
# inc <- p + aes(income, fill = Q) + geom_bar(position = "dodge")
# edu <- p + aes(PPEDUCAT, fill = Q) + geom_bar(position = "dodge")
# wor <- p + aes(work, fill = Q) + geom_bar(position = "dodge")
# mar <- p + aes(marital, fill = Q) + geom_bar(position = "dodge")
#
# grid.arrange(gen, age, eth)
# grid.arrange(inc, edu, wor, mar)

## plotting means with error bars
q <- svyby(~Q2, ~, des, svymean, na.rm = T)
ggplot(q, aes(, Q2Yes)) + geom_point() + xlab(" ") + ylab("% sick") +
