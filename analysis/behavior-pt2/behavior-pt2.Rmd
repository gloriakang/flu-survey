...
q11_df <- df %>%
  select(CaseID, PPGENDER, PPAGE, ppagecat, PPETHM, PPINCIMP, PPEDUC, PPEDUCAT,
         work, PPWORK, marital, PPMARITAL, PPMSACAT, ppreg9, PPSTATEN, PPHOUSE, PPRENT, PPNET,
         Q11_1:Q11_11, weight) %>%
  rename(Work = Q11_1,
         Schools = Q11_2,
         Day_care = Q11_3,
...
q11 <- data.frame(svytable(~Q11_q + Q11_r + PPGENDER + ppagecat + PPETHM +
                            PPINCIMP + PPEDUC + PPEDUCAT + work + PPWORK, des11, round = T))
# plot templates
...
p + geom_bar() + title
p + geom_bar() + aes(fill = Q11_r) + title
```
...
p2 + geom_bar(position = "fill") + aes(fill = Q11_r)

# select 'High Risk' response only
p3 <- ggplot(q11[q11$Q11_r == "High Risk", ], aes(weight = Freq)) + ptext
...
q11 <- data.frame(svytable(~Work + Schools + Day_care + Stores + Restaurants +
                             ...
                             Family_friends + Other + 
                             PPGENDER + ppagecat + PPETHM + PPINCIMP + EDUCAT, des11, round = T))
