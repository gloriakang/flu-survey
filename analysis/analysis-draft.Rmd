...
```{r Q13}
## Q13. Do you get the flu vaccine?
q13 <- as.data.frame(svytable(~Q13 + ppagecat +  + PPETHM + income + PPEDUCAT + work + marital, des, round = T))
names(q13)
...
# weighted table
svyby(~Q13, ~PPGENDER, des, svymean, na.rm = T)
...
df[c('CaseID', 'Q13','Q14')]
df[!is.na(df$Q14==T), c('CaseID', 'Q13','Q14')]

# drop NA
df_subset <- df[!is.na(df$Q14==T) & !is.na(df$Q13==T), ]
df_subset[c('CaseID','Q13','Q14')]
...
options(digits = 4)
options(survey.lonely.psu = "adjust")
des14 <- svydesign(ids = ~1, weights = ~weight, data = df_subset)
...
```{r}
## Q14. How much do you pay to get an influenza vaccine?
q14 <- as.data.frame(svytable(~Q14 + Q13, des14, round = T))
q14
...
# weighted table
(b <- svyby(~Q14, ~Q13, des14, svymean, na.rm.by = T, na.rm.all = T))
# unweighted table
with(df, addmargins(table(Q14, Q13)))


#svychisq(~Q14 + Q13, des14)

a <- glm(Q14 ~ Q13, family = quasibinomial(link = "logit"), data = df_subset, weights = weight)
summary(a)
```
...
q15 <- as.data.frame(svytable(
  ~Q15 + PPGENDER + ppagect4 + PPETHM + income + PPEDUCAT + work + marital, des14, round = T))
...
svyby(~Q15, ~Q13, des14, svymean, na.rm = T)
...
###
svytable(~Q15 + Q13, des14, round = T)
...
#####
svytable(~Q + Q13, des14, round = T)
svyby(~Q, ~Q13, des14, svymean, na.rm = T)
